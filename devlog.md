...
### やったこと
アプリ全体のUIをCRT/デジタルレトロ風から、スチームパンク・アナログ計器風デザインへ全面刷新した。

### 変更ファイル一覧
| ファイル | 変更内容 |
|---------|---------|
| `App.css` | CSSカラーパレット・全スタイルをスチームパンクテーマへ完全書き直し |
| `App.tsx` | ホームページのレイアウト切り替え構造を追加 |
| `DeskScene.tsx` | `raw`モード追加・ヘッダーに真空管SVGデコレーション追加 |
| `BottomNav.tsx` | ナビラベル「きろく」→「ホーム」へ変更 |
| `LogList.tsx` | table構造から縦リスト（div）に変更、右パネル向けにリデザイン |
| `CategoryTree.tsx` | カテゴリ項目に真鍮風ノブ装飾を追加 |
| `LogEditModal.tsx` | モーダルを白背景から木目ダーク背景に変更 |

### 新規ファイル
| ファイル | 内容 |
|---------|------|
| `VUMeter.tsx` | SVGで作成したアナログVUメーター（針アニメーション付き） |
| `HomePage.tsx` | 3パネル（日付+カテゴリ / VUメーター+時間+メモ / ログリスト）+ 気分ストリップのレイアウト |

### なぜこうしたのか

#### 全体テーマ
従来の「黒×緑のCRT画面風」から「暖色マホガニー木目×真鍮金属×アナログ計器風」へ。
配色変数を `:root` でまとめて管理しているため、変数を変えれば全コンポーネントに反映される設計。

#### VUメーター (SVG)
時間入力を「数字テキスト」から「アナログ針メーター」に変更。
- SVG `viewBox="0 0 340 182"` でアスペクト比を固定
- 針は `transform: rotate()deg` + `transformOrigin` + CSS `transition` でスムーズアニメーション
- メモリは 0〜60分の範囲で、5分刻みのマイナー目盛り・10分刻みのメジャー目盛り
- 針の回転角度: `-65° + (分/60) × 130°`

```tsx
// 回転角度計算の核心部分
const rot = useMemo(() => {
  const r = Math.max(0, Math.min(value, max)) / max;
  return -HS + r * (HS * 2); // HS = 65度
}, [value, max]);

// SVG要素への適用（CSS transitionでアニメーション）
<g style={{
...
#### 3パネルレイアウト
従来の縦スクロール1カラムから、固定高さの3カラムグリッドに変更。
- 左パネル: `flex: 0 0 210px`（固定幅）
- 中央パネル: `flex: 1`（残余スペースを埋める）
- 右パネル: `flex: 0 0 278px`（固定幅）

#### DeskSceneの`raw`モード
ホームページは独自レイアウトなのでスクロール可能なスクリーンラッパーを使いたくなかった。
`<DeskScene raw>` とするとスクリーンラッパーをバイパスして直接コンテンツを配置できる。

#### イベントバブリング対策
ホームページ全体を `<form>` で囲んでいるため、LogEditModal内のformのsubmitが
外側formにバブルアップして二重登録が起きる問題を `e.stopPropagation()` で対処。
